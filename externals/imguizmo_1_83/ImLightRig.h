...
namespace ImLightRig
{
   ...
   inline ImVec2 ImVec2Add(ImVec2 a, ImVec2 b)
   {
      return ImVec2(a.x + b.x, a.y + b.y);
   }

   inline ImVec2 ImVec2Sub(ImVec2 a, ImVec2 b)
   {
      return ImVec2(a.x - b.x, a.y - b.y);
   }
   ...
   {
      ImVec2 dif = ImVec2Sub(b, a);
      return std::sqrtf(dif.x * dif.x + dif.y * dif.y);
   }
   ...
   inline int Edit(Light* lights, int lightCount, int selected = -1, ImVec2 size = ImVec2(0,0))
   {
      ...
      draw_list->PushClipRect(canvas_pos, ImVec2Add(canvas_pos, frameSize));
      const float controlWidth = frameSize.x;
      
      draw_list->AddRectFilled(canvas_pos, ImVec2Add(canvas_pos, frameSize), 0xFF404040);
